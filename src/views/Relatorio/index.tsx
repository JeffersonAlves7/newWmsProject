...
export default function Relatorio() {
    ...
    useEffect(() => { api.get('/pedidos').then(({ data }) => setInfo({ pedidos: data.response })) }, [])

    function updateSituacao(situacao: UISituacao['situacao']) {
        if (situacao == statusData.situacao) return
        const url = infoToUrl({ situacao })

        api.get(url).then(({ data }) => setInfo({ pedidos: data.response, situacao }))
    }
    ...
    function updateIntegracao(integracao: UIFiltrarPedidos['integracao']) {
        ...
        api.get(url).then(({ data }) => setInfo({ pedidos: data.response, integracao }))
    }
    ...
    function updateData(data_: UIFiltrarPedidos["data"]) {
        ...
        api.get(url).then(({ data }) => setInfo({ pedidos: data.response, data: data_ }))
    }

    const ThCell = (props: { text: string | number }) => (<th>
        <div className="pb-2 pl-2 flex items-center justify-center"> <p>{props.text}</p> </div>
    </th>)
        ...
        <div className="w-[90%] flex-col flex gap-10">
            <header>
                ...
                <div className='flex justify-center flex-wrap items-center gap-2 print:hidden'>
                    ...
                    <div className=" p-1 rounded-lg border text-wmsPink  border-wmsBlack">
                        <Paragraph text={(statusData.pedidos.length + " pedidos encontrados").toUpperCase()} />
                    </div>
                </div>
            </header>
            <main className='w-full'>
                <table className="w-full">
                    ...
                    <tbody>
                        {statusData.pedidos.map(pedido => <RelatorioRow key={"relatorio_" + pedido.chavedeacesso} pedido={pedido} />)}
                    </tbody>
                </table >
            </main>
        </div>
