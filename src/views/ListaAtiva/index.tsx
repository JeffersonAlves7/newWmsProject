...
export default function listaAtiva() {
  const [info, setInfo] = useState<{
    ...
    total: number;
    pedidos: UIPedido[];
  }>({ embalado: 0, total: 0, pedidos: [] });
  const { id, error } = useParams();

  error && window.history.pushState("page2", "wmsPage2", `/listaAtiva/${id}`);

  const navigate = useNavigate();

  useEffect(() => {
    api
      .get("/pedidos?idLista=" + id)
      .then(({ data }) => {
        const { response } = data;

        response.filter(({ situacao }: UIPedido) => situacao == "finalizado")
          .length === response.length && navigate("/embalar");

        setInfo({
          embalado: response.filter(
            ({ situacao }: UIPedido) => situacao == "embalado"
          ).length,
  ...
  return (
    <div className="flex flex-col gap-[2rem] w-[90%] max-w-[700px]">
      ...
      <InputText
        id="input_lista"
        placeholder="Escanear ou inserir o ID do Pedido"
        onKeyDown={(e: KeyboardEvent) => {
          ...
          chaves.includes(value) && navigate(`/checkout/${value}`);
        }}
      />
      {error && (
