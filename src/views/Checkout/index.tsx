...
interface IUPedido { nf: string, pedido: string, itens: IUItem[], finalizado: boolean, }
...
export default function Checkout() {
    ...
    useEffect(() => {
        api.get(`/pedidos?chavedeacesso=${chavedeacesso}&itens=true`).then(({ data }) => {
            const { response } = data
            const new_itens: IUItem[] = []

            if (response[0].situacao !== "emaberto") navigator("/listaAtiva/" + response[0].idLista)
    ...
    function alterItemTotal(sku: IUItem['sku']) {
        ...
        const finalizado = itens.filter(item => item.quantidade === item.totalLidos).length === itens.length

        //Enviar informações
        setPedido({ nf: pedido.nf, pedido: pedido.pedido, itens: itens, finalizado: finalizado })
    ...
    const totalLidos = currentItem.totalLidos === currentItem.quantidade ? pedido.itens.indexOf(currentItem) + 1 : pedido.itens.indexOf(currentItem)

    return (
        <div className='w-[90%] flex max-w-[1000px] items-center gap-[1rem] sm:gap-[6rem] flex-col'>
            ...
            <main className="flex w-full  gap-4 flex-row items-center flex-wrap justify-center md:justify-start lg:justify-between">
                {pedido.finalizado === true
                    ? (
                        isAvailable === true ? (
                            <div className='h-[32rem] flex-col flex gap-10 w-full max-w-[23rem]'>
                                <iframe className="h-full w-full" src={services.api.baseURL + "/notas?nf=" + pedido.nf}></iframe>
                                <div className="w-[150px] grid">
                                    <WmsButton onClick={() => {
                                        (async () => {
                                            await api.put("/pedidos", { chavedeacesso: chavedeacesso, situacao: "embalado" })
                            ...
                            <div className='flex flex-col h-full gap-10' >
                                ...
                                <div className={isAvailable === false ? 'w-[150px] grid' : ""}>
                                    <NotaFiscal blank={false} cb={(v: boolean) => {
                                        if (v != true) return
                                        setIsAvailable(v)
                                    }} />
                                </div>
                    ...
                    : (
                        <div className="flex md:w-[50%] max-w-[24rem] h-full justify-between md:max-w-none flex-col gap-2">
                            <div className="grid grid-cols-2 gap-2">
                                <ItemImageLarger url={currentItem.imagem.url ? currentItem.imagem.url : currentItem.sku} />
                                <div className=' flex flex-col  justify-center gap-5  shadow-xl rounded-lg p-2 h-full'>
                                    ...
                                    <Paragraph text={"Descrição: " + currentItem.descricao + "..."} />
                                </div>
                            </div>
                            <InputText placeholder="Escanear ou inserir o SKU do Produto" id="input_checkout" onKeyDown={(e: KeyboardEvent) => {
                ...
                <Box lidos={totalLidos} total={pedido.itens.length} itens={pedido.itens} />
            </main >
        </div >
    )
}