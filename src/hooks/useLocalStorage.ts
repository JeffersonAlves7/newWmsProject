...
export default function useLocalStorage ( keyName:string, defaultValue:any){
  const [storedValue, setStoredValue] = React.useState(() => {  
    ...
    try { 
      ...
      if (value) { //Se o valor existir, retorna a string transformada em json
        return JSON.parse(value);
      } else { //Se não existir, cria um valor para o mesmo transformando o defaultvalue em string
  ...
  const setValue = (newValue: any) => { //Essa função será utilizada como um setState do useState
    try { //Apenas irá atualizar o valor no localStorage
